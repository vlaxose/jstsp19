function [H_with_beamSquint,Zbar,Ar,At,Dr,Dt] = wideband_mmwave_channel(L, Mr, Mt, total_num_of_clusters, total_num_of_rays, Gr, Gt)

% Initialization
H = zeros(Mr, Mt, L);
H_with_beamSquint = zeros(Mr, Mt, L);
Z = zeros(Gr, Gt, L);
...
Dt = 1/sqrt(Mt)*exp(-1j*(0:Mt-1)'*2*pi*(0:Gt-1)/Gt);
  ...
  for l=1:L
   ...
   Hl = zeros(Mr, Mt);
   Hl_with_beamSquint = zeros(Mr, Mt);
   index = 1;
   for tap = 1:total_num_of_clusters
       for ray=1:total_num_of_rays
           ...
           Ar(:, index, l) = angle(phi_r, Mr);
           At(:, index, l) = angle(genLaplacianSamples(1), Mt);

           Hl = Hl + rayleigh_coeff*Ar(:, index)*At(:, index)';
           Hl_with_beamSquint = Hl_with_beamSquint + rayleigh_coeff*(Ar(:, index))*At(:, index)';

           index = index + 1;
       end
       H(:,:,l) = H(:,:,l) + Hl;
       H_with_beamSquint(:,:,l) = H_with_beamSquint(:,:,l) + Hl_with_beamSquint;
   end

   H(:,:,l) = 1/sqrt(total_num_of_rays*total_num_of_clusters)*H(:,:,l);
   H_with_beamSquint(:,:,l) = 1/sqrt(total_num_of_rays*total_num_of_clusters)*H_with_beamSquint(:,:,l);
   Z(:,:,l) = Dr'*H(:,:,l)*Dt;
    end
