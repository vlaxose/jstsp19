...
for snr_indx = 1:length(snr_range)
  ...
  for np_indx=1:length(Np_range)
    total_num_of_rays = Np_range(np_indx);
    
    parfor r=1:maxMCRealizations
      
    disp(['Np = ', num2str(total_num_of_rays*total_num_of_clusters), ', realization: ', num2str(r)]);
...
%     disp('Running OMP...');
    ...
    mean_error_proposed(np_indx, snr_indx) = min(mean(error_proposed), 1);
    mean_error_omp(np_indx, snr_indx) = min(mean(error_omp), 1);
    mean_error_vamp(np_indx, snr_indx) = min(mean(error_vamp), 1);
    mean_error_twostage(np_indx, snr_indx) = min(mean(error_twostage), 1);
...
xlabel('Number of channel spatial paths');
ylabel('NMSE (dB)')
grid on;set(gca,'FontSize',12);

savefig('results/errorVSspatialpaths.fig')
