function [estFin,optFin,estHist] = Mini_AMP(scaEstIn, y, A, opt)
...
for n_b = 1:m/bl_size
    ...
    sub_blk = (n_b-1)*bl_size + 1:n_b*bl_size;
    A_sub   = A.A(sub_blk,:);
    
    % Compute Onsager-corrected residual
    A_sub = MatrixLinTrans(A_sub);
    Axhat = A_sub.mult(xhat);
        
    y_b   = y(sub_blk);
    
    % Continue with initialization: want (mean(Xvar,1)./rvarPrev)*vhatPrev = 0
    vhatPrev = zeros(bl_size,s);
    ...
    while ~stop
        ...
        % Compute Onsager-corrected residual
        vhat = scale*(y_b - Axhat) + bsxfun(@times,gain,vhatPrev);
        ...
        rhat = xhat + scale*A_sub.multTr(vhat); 
