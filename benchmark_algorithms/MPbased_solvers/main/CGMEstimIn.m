classdef CGMEstimIn < EstimIn
    ...
    methods
        ...
        % Gaussian components
        function obj = CGMEstimIn(omega, theta, phi, varargin)
            obj = obj@EstimIn;
            obj.omega = omega;
            obj.theta = theta;
            obj.phi = phi;            
        ...
        function set.disableTune(obj, flag)
            assert(isscalar(flag), ...
                'CGMEstimIn: disableTune must be a logical scalar');
        ...
        function [xhat, xvar, NKL] = estim(obj, Rhat, Rvar)
            ...
            xvar = sum((nu + abs(gamma).^2)./lik,3) - abs(xhat).^2;
        
            % Autotune the CGM parameters via approzimate expectation
            % maximization            
            if obj.autoTune && ~obj.disableTune
              ...
              if (obj.counter>0), % don't tune yet
                ...
                switch obj.tuneDim
                    ...
                    otherwise
                         error('Invalid tuning dimension in CGMEstimIn');
                end
            ...
            if (nargout >= 3)                            
                ...
                zeta(zeta == 0) = eps;
                NKL = log(zeta)+ log(pi*Rvar)+(xvar + abs(xhat - Rhat).^2)./(Rvar);
            end
