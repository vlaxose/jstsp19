...
% -------------------------------
%
function [X,Y,S,Out,time_data,estHist] = lmafit_sms_v1_timing(D,k,opts,beta,error_function)
%
...
for iter = 1:maxit
    ...
    %Save result
    if ~transposed
        estHist.errZ(iter) = error_function(Dscale*X*Y);
    else
        estHist.errZ(iter) = error_function(Dscale*Y'*X');
    end
