function [ precision_recall ] = calc_precision_recall( video_path, frames_vec, frame_names_gt, fgs_cmp, MAX_gt )
%CALC_PRECISION_RECALL Summary of this function goes here
%   Detailed explanation goes here
threshvec=linspace(0,1,100);
precision_recall=zeros(length(threshvec),2);
tidx_tot = 0; N=0;
...
for t=1:length(threshvec)
    ...
    precision_recall(t,1)= tp_tot(t)/(tp_tot(t)+fp_tot(t));  % precision = tp/(tp+fp)
    precision_recall(t,2)= tp_tot(t)/(tp_tot(t) + fn_tot(t)); % recall
end
