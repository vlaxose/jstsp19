function [X,Y,Out,time_data,estHist] = lmafit_mc_adp_timing(m,n,k,Known,data,opts,error_function)
%
...
%   modified by Jason T. Parker 03/18/2012 for timing purposes
...
maxit = 500;
iprint = 1;
%Parker change on 16 May 2012 for comparisons at 512x512
...
%Preallocate storage
estHist.errZ = zeros(maxit,1);
time_data = zeros(maxit,1);
...
for iter = 1:maxit
    ...
    %Save result
    estHist.errZ(iter) = error_function(X*Y);
