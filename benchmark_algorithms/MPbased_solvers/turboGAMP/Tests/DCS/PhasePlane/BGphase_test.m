% BGPHASE_TEST        A test of EMturboGAMP for the DCS signal model,
...
%
% SYNTAX:
% BGphase_test(N, T, p01, Alpha, CommonA, SNRmdB, N_trials, fid)
%
% INPUTS:
...
function BGphase_test(N, T, p01, Alpha, CommonA, SNRmdB, N_trials, fid)

%% Declare the parameters for the signal model and test setup

if nargin == 0
    N = 512;             	% # parameters                              [dflt=512]
    T = 25;                  % # of timesteps                          	[dflt=25]
    p01 = 0.05;             % Pr{S(n,t) = 0 | S(n,t-1) = 1}             [dflt=0.05]
    Alpha = 0.01;           % Innovation rate of thetas (1 = total)     [dflt=0.01]
    CommonA = false;      	% Use a single A matrix for all timesteps?  [dflt=false]
...
savefile = [datestr(now, 'ddmmmyyyy_HHMMSSFFF') '_BGphase_test_N_' ...
    num2str(N) '_T_' num2str(T) '_p01_' num2str(p01) '_Alpha_'  num2str(Alpha) ...
...
%                 'learn_active_var', 'scalar', 'init_params', 'false');
            ...
            % Create a SupportStruct class object for DCS model
            SupportObj = MarkovChain1('p01', p01, 'learn_p01', 'true', ...
                'dim', 'row');
            ...
            % Create Observation class object for AWGN
            ObservationObj = GaussNoise('learn_prior_var', 'scalar', ...
                'init_params', 'true');
            ...
            TurboObj = TurboOpt('Signal', SignalObj, 'Observation', ...
                ObservationObj, 'SupportStruct', SupportObj, ...
                'AmplitudeStruct', AmplitudeObj, 'RunOptions', RunOptionsObj);
            
            % Create the signal, measurements, matrices, etc.
            [Y, A, X, S, NewObserObj] = signal_gen_fxn(GenParamObj, TurboObj);
            
            % Replace initial NoiseObj with NewNoiseObj, since it contains
            % the true noise variance
            ObservationObj = NewObserObj;
            TurboObj.Observation = ObservationObj;
            sig2e = ObservationObj.prior_var;
            ...
            % NSER
            NSER_sks(b,d,n) = nser(unique(cat(1, support{:})), ...
                find(sum(abs([X_sks{:}]),2) > 1e-2));
...
%                 'learn_active_var', 'scalar', 'init_params', 'false');
            ...
            NaiveObservObj = GaussNoise('learn_prior_var', 'scalar', ...
                'init_params', 'false', 'prior_var', sig2e);
            
            NaiveTurboObj = TurboOpt('Signal', NaiveSignalObj, 'Observation', ...
                NaiveObservObj, 'SupportStruct', NoSupportStruct(), ...
            ...
            % NSER
            NSER_naive(b,d,n) = nser(unique(cat(1, support{:})), ...
                find(sum(abs(X_naive),2) > 1e-2));
...
%                 'learn_active_var', 'false', 'init_params', 'false');
            ...
            % Create a SupportStruct class object for DCS model
            NoEMSupportObj = MarkovChain1('p01', p01, 'learn_p01', 'false', ...
                'dim', 'row');
            ...
            % Create Noise class object for AWGN
            NoEMObserObj = GaussNoise('learn_prior_var', 'false', ...
                'init_params', 'false', 'prior_var', sig2e);
            
            % Place signal model objects into TurboOpt container
            NoEMTurboObj = TurboOpt('Signal', NoEMSignalObj, 'Observation', ...
                NoEMObserObj, 'SupportStruct', NoEMSupportObj, ...
            ...
            % NSER
            NSER_noem(b,d,n) = nser(unique(cat(1, support{:})), ...
                find(sum(abs(X_noem),2) > 1e-2));
            ...
            % NSER
            NSER_turbo(b,d,n) = nser(unique(cat(1, support{:})), ...
                find(sum(abs(X_turbo),2) > 1e-2));
