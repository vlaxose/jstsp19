% PHASE_TEST        A test of EMturboGAMP for the MMV signal model,
...
%
% SYNTAX:
% phase_test(N, T, alpha, CommonA, SNRmdB, N_trials, fid)
%
% INPUTS:
...
function phase_test(N, T, Alpha, CommonA, SNRmdB, N_trials, fid)
...
if nargin == 0
    ...
    CommonA = true;      	% Use a single A matrix for all timesteps?  [dflt=true]
...
%             D = 1;      % 1 mixture component
            ...
            D = 2;
            SignalObj = GaussMix('sparsity_rate', cat(3, lambda/D, lambda/D), 'active_mean', ...
                cat(3, Theta, -Theta), 'active_var', cat(3, Sigma2, Sigma2), 'learn_sparsity_rate', ...
            ...
            y_cell = cell(1,T);  A_cell = cell(1,T);
            for t = 1:T
                y_cell{t} = Y(:,t);
                switch CommonA
            ...
            tic;
            % Solve using the SKS
            [X_sks] = genie_mmv_fxn(y_cell, A_cell, support, ...
