...
GAMP_options.nit = 10000;	% make sure we don't terminate too early [10000]
...
%GAMP_options.adaptStep = 0;	% definitely adapt the stepsize [1]
...
GAMP_options.stepWindow = 1;	% allow alg to take a few temporarily bad steps [20]
...
%Output channel
outputEst = AwgnEstimOut(y, muw,true);
...
GAMP_options.xvar0 = 0.25/(Lam_target/2/sqrt(2*muw))*norm(y)/norm(A,'fro')*sqrt(N/M);
...
%TFOCS_options.alg= 'AT'; TFOCS_options.restart= -Inf;
TFOCS_options.alg= 'N07'; TFOCS_options.restart= -10;
TFOCS_options.tol = GAMP_options.tol;
tic;
[resTFOCS, output] = tfocs( smooth_quad, { A, -y }, prox_l1( Lam_target*sqrt(muw)/2 ) ,[], TFOCS_options);
tTFOCS = toc
LamTFOCS = 2*max(abs(A'*(A*(resTFOCS) - y)))/sqrt(muw)
if (p==1)&(abs(LamTFOCS-Lam_target)/Lam_target>0.05),
    warning(['Lam_target=',num2str(Lam_target),' whereas Lam_TFOCS=',num2str(LamTFOCS)])
end;
...
errTFOCS = norm(x - resTFOCS) / norm(x);
...
figure(1); clf
 ...
 plot(real(resTFOCS),'gs')
 plot(real(resGAMP),'bx')
 plot(real(resGAMP2),'m^')
 hold off
 ylabel('real')
 legend('Truth','FISTA','TFOCS','GAMP-DMM','GAMP-MAP')
 grid on;
 if strcmp(Atype,'iid')
...
subplot(311)
 ...
 title(['NMSE: '...
	...
	',  GAMP-MAP=' num2str(20*log10(errGAMP2(end))),' dB'...
    	...
    	',  TFOCS=' num2str(20*log10(errTFOCS(end))),'dB'...
...
subplot(312)
 ...
 title(['k-true=' num2str(sum(x~=0)),...
	...
	',  k-GAMP-MAP=' num2str(sum(resGAMP2~=0)),...
    	...
    	',  k-TFOCS=' num2str(sum(resTFOCS~=0)),...
    ]);
subplot(313)
